...
$(document).ready(function(){
	...
	$("#foldNode").click(function(event){
		if (currentNode.hasLeaves == true) {
			for (var i = 0; i < resultNodeArr.length; i++) {
				if (resultNodeArr[i].parentId == currentNode.id) {
					//	隐藏节点
					resultNodeArr[i].visible =false;
					for (var l = 0; l < resultLinkArr.length; l++) {
						if (resultLinkArr[l].id == resultNodeArr[i].id) {
							//	隐藏连线
					...
					//	如果当前节点还有下一层
					if (resultNodeArr[i].hasLeaves == true) {
		...
		// eachNode(currentNode, resultNodeArr); 
		
	})

	function eachNode(currentHideNode, nextNodeArr){
		console.log("currentHideNode================>", currentHideNode.id)
		for (var i = 0; i < nextNodeArr.length; i++) {
			// 找到当前操作的节点下所有的子节点
			if (nextNodeArr[i].parentId == currentHideNode.id) {
				for (var l = 0; l < resultLinkArr.length; l++) {
					//	找到当前操作的节点下子节点的连线
					if (resultLinkArr[l].id == nextNodeArr[i].id) {
						//	隐藏该节点下的子节点和连线
						nextNodeArr[i].visible =false;
						resultLinkArr[l].visible =false;
						resultEachArr.push(resultNodeArr[i]);
					}
				}
			}
		}

		console.log("resultEachArr==============>", resultEachArr)
		// for (var p = 0; p < resultEachArr.length; p++) {
		// 	if (resultEachArr[p].hasLeaves == true) {	
		// 		eachNode(resultEachArr[p], resultNodeArr);
		// 	}			
		// }
	...
	// 		// 	// 判断它是否还有子节点
	// 		// 	if (resultNodeArr[i].hasLeaves == true) {//	还有子节点
	// 		// 		// 找到当前操作的节点下所有的子节点
	// 		// 		if (resultNodeArr[i].parentId == currentNode.id) {
	// 		// 			for (var l = 0; l < resultLinkArr.length; l++) {
	// 		// 				//	找到当前操作的节点下子节点的连线
